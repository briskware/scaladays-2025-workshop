...
object CreateCompanyPage
    extends FormPage[CreateCompanyState]("Post New Company") {
  ...
  override def renderChildren(): List[ReactiveHtmlElement[Element]] =
    List(
      ...
      div(
        cls := "d-flex justify-content-between mb-3",
        button(
          `type` := "button",
          "Post ",
          onClick.preventDefault.mapTo(stateVar.now()) --> submitter
        )
        // TODO add another button for the paid version
      )
    )
        ...
        div(
          ...
          div(
            ...
            img(
              ...
              src <-- stateVar.signal.map(
                _.image.getOrElse(Constants.companyLogoPlaceholder)
              )
            )
          )
        )
      ...
      reader.onload = _ => {
        ...
        fakeImage.addEventListener(
          ...
          _ => {
            ...
            val (width, height) =
              computeDimensions(fakeImage.width, fakeImage.height)
            canvas.width = width
